...
Each collection has its own identifier af69e45f-646c-43f4-b062-b2855b5d6b5f and _43f68ebb-e498-416d-9bcd-216aa4580c3a_ respectively. 
...
You can also restrict the download to a given bounding box. This is done by passing an array of 4 coordinates. The values of the bounding box must be the coordinates of a rectangle in the `EPSG:4326` projection.

The order of the 4 coordinates is as follows: 
_BoundingBox: [`max.lat`,`max.lon`,`min.lat`,`min.lon`] which is the same as [`N`,`E`,`S`,`W`]_
...
```{http:example} curl wget python-requests
    :request: ./download-request-download-timeseries.req
```
...
If no coordinates are entered, the used bounding box will be the following:
...
If no start and end date are entered for WEkEO datasets, the used ones will be the following:
